...
public sealed class MidiMapConfigurationStore
{
    ...
    public MidiMapConfiguration CreateConfiguration(BankSet bankSet)
    {
        return new MidiMapConfiguration
        {
            Banks = bankSet.Banks.Select(CreateBankConfiguration).ToList()
        };
    }

    public async Task SaveAsync(BankSet bankSet, string path, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<MidiMapConfiguration> LoadAsync(string path, CancellationToken cancellationToken = default)
    {
    ...
    public void Apply(BankSet bankSet, MidiMapConfiguration configuration)
    {
    ...
    private static BankMapConfiguration CreateBankConfiguration(ControlBank bank)
    {
        return new BankMapConfiguration
        {
            ...
            Slots = bank.Slots.Select(CreateSlotConfiguration).ToList()
        };
    }
