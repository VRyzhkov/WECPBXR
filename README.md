...
GitHub release assets include `.sha256` files generated by the release workflow. A checksum verifies that the downloaded file matches the published release asset.

On Windows, download the asset and its matching `.sha256` file, then compare the printed hash:

```powershell
Get-FileHash .\VRyzhkov.WECPBXR.UI-win-Setup.exe -Algorithm SHA256
Get-Content .\VRyzhkov.WECPBXR.UI-win-Setup.exe.sha256
```

On Linux, download the asset and its matching `.sha256` file into the same directory and run:

```bash
sha256sum -c ./downloaded-release-file.sha256
```

Use the exact `.sha256` file name published next to the downloaded release asset.
...
К каждому файлу в GitHub Release добавляется файл `.sha256`, который формируется release workflow. Контрольная сумма позволяет проверить, что скачанный файл совпадает с опубликованным артефактом.

В Windows скачайте файл релиза и соответствующий `.sha256`, затем сравните выведенный хеш:
...
В Linux скачайте файл релиза и соответствующий `.sha256` в одну папку и выполните:

```bash
sha256sum -c ./downloaded-release-file.sha256
```

Используйте точное имя `.sha256`, опубликованное рядом со скачанным файлом релиза.
