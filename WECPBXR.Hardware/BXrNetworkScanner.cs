...
public sealed class BXrNetworkScanner
{
    private static readonly TimeSpan DefaultTimeout = TimeSpan.FromSeconds(2);

    public static async Task<IReadOnlyList<BXrDiscoveredMixer>> ScanAsync(CancellationToken cancellationToken = default)
    {
        return await ScanAsync(DefaultTimeout, cancellationToken).ConfigureAwait(false);
    }

    public static async Task<IReadOnlyList<BXrDiscoveredMixer>> ScanAsync(
        TimeSpan timeout,
        CancellationToken cancellationToken = default)
    {
        ...
        return [.. foundMixers
            ...
            .Select(pair => new BXrDiscoveredMixer(pair.Key, pair.Value))];
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        IPAddress[] targets = [.. GetSlash24Targets(localAddress)];

        foreach (IPAddress target in targets)
        {
            cancellationToken.ThrowIfCancellationRequested();
            await udpClient.SendAsync(request, request.Length, new IPEndPoint(target, BXrConnectionSettings.DefaultOscPort))
